function salvarBanco(Aposta,Variavel)
...
if any(strcmp(Variavel.Mesa(1:3),''))
    ...
    %try [~,Posicao] = ismember(Resultado,Jogos,'rows');
    %catch, Posicao = size(Jogos,1);
    %end
...
Entrada = ([Probabilidade Stack Viradas NPartida ...
            NRaise Posicao BigBlind ...
            NFold StackMin Pot PotTotal Dealer NJogo]);

Saida = min([max([(Aposta/Variavel.Stack(Variavel.Jogador)) 0]) 1]);

if ~exist(['bd\' Variavel.Hora '.txt'],'file')
    try Arquivo = fopen(['bd\' Variavel.Hora '.txt'],'a'); catch, end
    try fprintf(Arquivo,['PROBAB  STACK VIRADA NPARTI BIGBLI NRAISE  NFOLD ' ...
                         'POSICA    POT STACKM POTTOT DEALER  NJOGO  SAIDA \r\n']); catch, end
else
    try Arquivo = fopen(['bd\' Variavel.Hora '.txt'],'a'); catch, end
end

try fprintf(Arquivo,[repmat('%0.4f ',1,numel([Entrada Saida])) '\r\n'],[Entrada Saida]); catch, end

try fclose(Arquivo); catch, end